...
,,ModelNet2" and then trains neural network.
'''
...
def create_model(configuration):
    ...
    weights = {'W_conv1': tf.Variable(tf.random_normal([5, 5, config.N_DEPTH, 32])),
               'W_conv2': tf.Variable(tf.random_normal([5, 5, 32, 64])),
               'W_fully_connected1': tf.Variable(tf.random_normal(
                [s_rows*s_columns*64, 1024])),
    ...
    # Input: [None, 19, 22, 64]
    # Output: [None, 26752]
    flatten1 = tf.reshape(pool2, [-1, s_rows*s_columns*64])
