...
def create_model(configuration):
    ...
    weights = {'W_conv1': tf.Variable(tf.random_normal([1, 11, config.N_DEPTH, 32])),
               'W_conv2': tf.Variable(tf.random_normal([1, 5, 32, 64])),
               'W_conv3': tf.Variable(tf.random_normal([1, 3, 64, 96])),
               'W_conv4': tf.Variable(tf.random_normal([1, 3, 96, 128])),
               'W_conv5': tf.Variable(tf.random_normal([1, 3, 128, 256])),
               'W_fully_connected1': tf.Variable(tf.random_normal(
                    [1 * s_columns * 256, 4096])),
    ...
    flatten1 = tf.reshape(pool3, [-1, 1 * s_columns * 256])
