...
namespace Scene3D
{
    ...
    public partial class SceneBrep : ICloneable
    {
        ...
        /// <summary>
        /// Delete a vertex defined by its 3D coordinate.
        /// </summary>
        /// <param name="v">Vertex coordinate in the object space</param>
        /// <returns>Vertex handle</returns>
        public void DeleteVertex ( Vector3 v )
        {
            geometry.Remove( v );
        }

        public void FreeTriangleSpace ( int c )
        {
            vertexPtr[ c ] = -1;
            vertexPtr[ c + 1 ] = -1;
            vertexPtr[ c + 2 ] = -1;
        ...
        /// 
        /// </summary>
        /// <param name="c">corner hande</param>
        public void DeleteFreeSpace ( )
        {
            List<int> newVertexPtr = new List<int>();

            foreach ( var item in vertexPtr )
            {
                if ( item != -1 )
                    newVertexPtr.Add( item );
            }
    ...
    /// <returns>Object-space coordinates</returns>
        ...
        /// <summary>
        /// Takes vertex pointer as an argument and returns 
        /// </summary>
        /// <param name="ptr"></param>
        /// <returns></returns>
        public List<int> GetTriangleFromPtr ( int ptr )
        {
            int tr = 0;
            var list = new List<int>();

            for ( int i = 0; i < vertexPtr.Count; i++ )
            {
                if ( vertexPtr[i] == ptr )
                {
                    if ( i % 3 == 0 )
