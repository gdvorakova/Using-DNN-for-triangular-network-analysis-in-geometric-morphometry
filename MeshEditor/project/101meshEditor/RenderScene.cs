/* Copyright (C) 2017 Josef Pelikan
...
namespace _101meshEditor
{
    public partial class Form1
    {
                    ...
                    else
                    {
                        ...
                        if (triangleSelection.Checked)
                        {
                            commandManager.Execute(
                                new Scene3D.TriangleSelectionCommand(
                                    selectedPoints, triangleIndex));
                        ...
                        else if ( pointSelection.Checked )
                        {
                            commandManager.Execute(
                                new Scene3D.PointSelectionCommand(
                                    selectedPoints, uvSpot, triangleIndex));
                            if (pointLogFile != null)
                            {
                                if (pointLogFile != null)
                                {
                                    pointLogFile.WriteLine(
                                        selectedPoints.SelectionPointers[selectedPoints.SelectionPointers.Count - 1]);
                                }
                            }
                        ...
                        else if ( brushSelection.Checked )
                        {
                            commandManager.Execute(
                                new Scene3D.BrushSelectionCommand(
                                    selectedPoints, uvSpot, triangleIndex, 40.0f));
                            if (commandManager.UndoCount > 0)
                                undoToolStripMenuItem.Enabled = true;
                            else undoToolStripMenuItem.Enabled = false;
                        }
        ...
        new GlShaderInfo( ShaderType.VertexShader, "vertex.glsl", "101meshEditor" ),
        new GlShaderInfo( ShaderType.FragmentShader, "fragment.glsl", "101meshEditor" ) } );
            programs[ pi.name ] = pi;
